...
def get_file_data_for_pf(filename):
    return get_replay_data(filename)


def get_replay_data(filename):
    """Load a WP-ICP log and return [(t, control_signal, robot_sensor_signal), ...]."""
    data_loader = robot_python_code.DataLoader(filename)
    data_dict = data_loader.load()

    time_list            = data_dict['time']
    control_signal_list  = data_dict['control_signal']
    sensor_signal_list   = data_dict['robot_sensor_signal']

    t0   = time_list[0]
    rows = []
    for i in range(len(time_list)):
        rows.append([time_list[i] - t0, control_signal_list[i], sensor_signal_list[i]])
    return rows
